...
const HIGH_VALUE_EVENTS = new Set<RankingEventType>([
...
const COMMERCIAL_EVENTS = new Set<RankingEventType>([
...
export class EventIngestionService {
  static async ingest(dto: IngestEventDto): Promise<IRankingRawEvent> {
    ...
    const confidence = this.computeEventConfidence(dto as IngestEventDto & { event_type: RankingEventType }, timestamp);
  ...
  static async ingestBatch(events: IngestEventDto[]): Promise<{ accepted: number; rejected: number }> {
    let accepted = 0;
    let rejected = 0;
  ...
  private static computeEventConfidence(
    dto: IngestEventDto & { event_type: RankingEventType },
    timestamp: Date
  ): number {
    let base = 0.5;

    // High-value interactions get baseline boost
    if (HIGH_VALUE_EVENTS.has(dto.event_type)) base = 0.75;
    if (COMMERCIAL_EVENTS.has(dto.event_type)) base = 0.80;
    ...
    if (dto.duration_ms !== undefined) {
      if (dto.duration_ms < 0) durationQuality = 0;
      else if (dto.duration_ms > 60 * 60 * 1000) durationQuality = 0.5; // >1hr unrealistic
    }
  ...
  private static async upsertSession(event: IRankingRawEvent, confidence: number): Promise<void> {
    const INACTIVITY_LIMIT_MS = 30 * 60 * 1000;
    const now = event.timestamp;

    const existing = await RankingSession.findOne({ session_id: event.session_id });

    if (!existing) {
      const hasCommercial = COMMERCIAL_EVENTS.has(event.event_type as RankingEventType);
      const hasComparison = (event.event_type === 'compare_open' || event.event_type === 'compare_interaction' || event.event_type === 'variant_compare');
      await RankingSession.create({
    ...
    if (msSinceLastEvent > INACTIVITY_LIMIT_MS) {
      await RankingSession.updateOne(
        { session_id: event.session_id },
        { is_active: false, ended_at: existing.last_event_at }
      );
    ...
    const hasCommercial = existing.has_commercial_intent || COMMERCIAL_EVENTS.has(event.event_type as RankingEventType);
