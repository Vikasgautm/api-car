import { IRankingRawEvent, RankingEventType } from '../../../models/ranking-raw-event.model';
import { IRankingSession, RankingSession } from '../../../models/ranking-session.model';
...
export class SessionQualityService {
  static async computeForSession(sessionId: string, events: IRankingRawEvent[]): Promise<QualityResult> {
  ...
  private static computeAttentionStrength(events: IRankingRawEvent[]) {
  ...
  private static computeTimeConfidence(events: IRankingRawEvent[]): number {
  ...
  private static computeScrollConfidence(events: IRankingRawEvent[]): number {
  ...
  private static computeRevisitConfidence(events: IRankingRawEvent[]): number {
    const returnVisits = events.filter(e => e.event_type === 'return_visit');
    if (returnVisits.length === 0) return 0;
    return Math.min(7, returnVisits.length * 3.5);
  }

  // ── EXPLORATION STRENGTH ───────────────────────────────────────────────────

  private static computeExplorationStrength(events: IRankingRawEvent[]) {
  ...
  private static computeSpecDepth(events: IRankingRawEvent[]): number {
    const specEvents = events.filter(e => e.event_type === 'spec_interaction');
    if (specEvents.length === 0) return 0;
  ...
  private static computeGalleryDepth(events: IRankingRawEvent[]): number {
  ...
  private static computeFaqDepth(events: IRankingRawEvent[]): number {
    const expands = events.filter(e => e.event_type === 'faq_expand').length;
    const reads = events.filter(e => e.event_type === 'faq_read').length;
    if (expands === 0 && reads === 0) return 0;
    return Math.min(5, expands * 1 + reads * 2);
  }

  private static computeFeatureToolDepth(events: IRankingRawEvent[]): number {
  ...
  private static computeEvaluationStrength(events: IRankingRawEvent[]) {
  ...
  private static computeComparisonDepth(events: IRankingRawEvent[]): number {
  ...
  private static computeVariantAnalysis(events: IRankingRawEvent[]): number {
  ...
  private static computeRepeatEvaluation(events: IRankingRawEvent[]): number {
    const evalTypes: RankingEventType[] = ['spec_interaction', 'variant_compare', 'compare_interaction'];
    const evalEvents = events.filter(e => evalTypes.includes(e.event_type as RankingEventType));
  ...
  private static computeCommercialStrength(events: IRankingRawEvent[]) {
  ...
  private static computeEmiConfidence(events: IRankingRawEvent[]): number {
  ...
  private static computeBrochureConfidence(events: IRankingRawEvent[]): number {
  ...
  private static computeDealerConfidence(events: IRankingRawEvent[]): number {
  ...
  private static computeNoisePenalty(allEvents: IRankingRawEvent[], validEvents: IRankingRawEvent[]): number {
    let penalty = 0;

    // Bounce: single page_view with no meaningful interaction
    if (allEvents.length <= 2 && validEvents.length === 0) penalty += 10;

    // Very low validation rate = spam/bot traffic
    const validRatio = allEvents.length > 0 ? validEvents.length / allEvents.length : 0;
    if (validRatio < 0.2 && allEvents.length > 5) penalty += 15;
    ...
    if (allEvents.length > 3) {
      ...
      if (eventsPerSecond > 2) penalty += 20; // >2 events/sec is suspicious
    }
  ...
  static async recomputeAndSave(sessionId: string): Promise<void> {
    const events = await import('../../../models/ranking-raw-event.model').then(m =>
      m.RankingRawEvent.find({ session_id: sessionId }).lean()
    );
    if (events.length === 0) return;

    const result = await this.computeForSession(sessionId, events);

    await RankingSession.updateOne(
  ...
  private static emptySignals(): IRankingSession['signals'] {
    return {
