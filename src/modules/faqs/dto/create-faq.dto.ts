...
export class CreateFaqDto {
  ...
  static validate(dto: CreateFaqDto): { valid: boolean; errors: string[] } {
    ...
    const qResult = ValidationUtil.required(dto.question, 'question');
    ...
    const aResult = ValidationUtil.required(dto.answer, 'answer');
    if (!aResult.valid) errors.push(...aResult.errors);
    if (dto.answer && dto.answer.length < 20) errors.push('answer must be at least 20 characters long');

    const cResult = ValidationUtil.required(dto.category, 'category');
    if (!cResult.valid) errors.push(...cResult.errors);
