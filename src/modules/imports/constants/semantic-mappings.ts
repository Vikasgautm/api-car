...
export interface SemanticMapping {
  ...
  label: string;
  synonyms: string[];
  confidence_boost?: number; // Optional: boost confidence if matched (0.05-0.2)
}
 ...
 */
export function buildSemanticLookupTable(): Map<string, string> {
 ...
 */
export function getSemanticMapping(key: string): SemanticMapping | undefined {
  return SEMANTIC_MAPPINGS.find(m => m.key === key);
}

/**
 * Get all mappings for a category
 */
export function getSemanticMappingsByCategory(category: string): SemanticMapping[] {
  return SEMANTIC_MAPPINGS.filter(m => m.category === category);
}
