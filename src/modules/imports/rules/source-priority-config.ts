...
export interface SourcePriority {
  source: SourceType;
  priority: number; // Higher = more trusted (0-100)
  category: 'aggregator' | 'official' | 'user';
}

export interface FieldSourceTrust {
  field: string; // e.g., "specs_normalized.engine_performance.max_power"
  trusted_sources: SourceType[]; // Ordered by trust
}
...
export const FIELD_SOURCE_TRUST: FieldSourceTrust[] = [
  {
...
export class SourcePriorityEngine {
  static getPriorityForSource(source: SourceType): number {
    const priority = SOURCE_PRIORITIES.find(s => s.source === source);
    return priority?.priority ?? 0;
  }

  static getHighestPrioritySource(...sources: SourceType[]): SourceType {
    return sources.sort(
      (a, b) => this.getPriorityForSource(b) - this.getPriorityForSource(a)
    )[0];
  }

  static shouldUseSourceForField(source: SourceType, field: string): boolean {
    const fieldTrust = FIELD_SOURCE_TRUST.find(f => f.field === field);
    if (fieldTrust) {
  ...
  static getTrustedSourcesForField(field: string): SourceType[] {
    ...
    return SOURCE_PRIORITIES.map(s => s.source);
  }

  static mergeSpecsFromMultipleSources(
    specsBySource: Map<SourceType, Record<string, any>>
  ): Record<string, any> {
    const merged: Record<string, any> = {};

    // For each source in priority order, fill gaps
    const prioritizedSources = Array.from(specsBySource.keys()).sort(
      (a, b) => this.getPriorityForSource(b) - this.getPriorityForSource(a)
    );
  ...
  private static deepMergeSpecs(target: any, source: any, sourceType: SourceType): void {
