...
export class CarWaleExtractor {
  ...
  private static async fetchHtml(url: string): Promise<string> {
    if (!url.includes('carwale.com')) {
      throw new AppError('Unsupported source. Only carwale.com URLs are supported.', 400);
    }
    try {
      ...
      if (error.code === 'ECONNABORTED') throw new AppError('Request timeout. The page took too long to load.', 408);
      if (error.response?.status === 404) throw new AppError('Page not found. Please check the URL.', 404);
  ...
  private static parsePrice(priceText: string): number | null {
    ...
    if (cleaned.includes('lakh')) {
      ...
      if (m) return Math.round(parseFloat(m[0]) * 100000);
    }
    if (cleaned.includes('crore') || cleaned.includes(' cr')) {
      const m = cleaned.match(/\d[\d.]*/);
      if (m) return Math.round(parseFloat(m[0]) * 10000000);
    }
    const m = cleaned.match(/\d[\d,]*/);
  ...
  private static parseJsonLd(html: string): any {
    const matches =
    ...
    for (const m of matches) {
      ...
      try {
        ...
        const nodes: any[] = data['@graph'] ? data['@graph'] : [data];
        const carNode = nodes.find((n: any) => n['@type'] === 'Car');
   ...
   */
  private static parseSpecs($: cheerio.CheerioAPI): ExtractedSpec[] {
    ...
    const addSpec = (section: string, label: string, value: string) => {
      ...
      if (!label || !value || label.length > 150) return;
      ...
      const seatAdjMatch = label.match(
        /^(driver|front passenger|second row|third row)\s+seat adjustment\s*[:–\-]/i
      );
      if (seatAdjMatch) {
  ...
  static async extractCarData(url: string): Promise<ExtractedCarData> {
    ...
    let bodyType: string = jsonLd?.bodyType || '';
    if (!bodyType) {
      const descText = jsonLd?.description || bodyText;
      const m = descText.match(
        /\b(Sub-Compact SUV|Compact SUV|SUV|Sedan|Hatchback|MUV|MPV|Coupe|Convertible|Pickup Truck)\b/i,
      );
      if (m) bodyType = m[1];
    }
  ...
  static async extractVariantData(url: string): Promise<ExtractedVariantData> {
    ...
    if (!variantName && jsonLd?.name) {
      ...
      variantName = this.cleanText(
        jsonLd.name.replace(brandStr, '').replace(modelStr, ''),
      );
    }
    if (!variantName) variantName = this.cleanText($('h1').first().text());
    ...
    const fuelTypeFromSpec = specs.find(
      s => s.label.toLowerCase() === 'fuel type',
    )?.value || '';
    const fuelType: string = fuelTypeFromSpec || jsonLd?.fuelType || '';

    // Transmission: prefer spec row "Transmission" (e.g., "Manual - 5 Gears") then JSON-LD.
    const transmissionFromSpec = specs.find(
      s => s.label.toLowerCase() === 'transmission',
    )?.value || '';
    const transmissionRaw = jsonLd?.vehicleTransmission;
    const transmissionFromJsonLd: string =
      typeof transmissionRaw === 'string'
        ? transmissionRaw
        : typeof transmissionRaw?.name === 'string'
          ? transmissionRaw.name
          : '';
    const transmission: string = transmissionFromSpec || transmissionFromJsonLd;
    ...
    fs.writeFileSync(
      logFile,
      JSON.stringify(
        { url, variantName, fullName, price, priceText, fuelType, transmission, specs_count: specs.length, specs },
        null,
        2,
      ),
      'utf-8',
