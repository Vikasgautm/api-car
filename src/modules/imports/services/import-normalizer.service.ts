...
import { SemanticNormalizer } from '../normalizers/semantic-normalizer';
...
export class ImportNormalizerService {
  ...
  static normalize(specs_raw: Record<string, any> = {}): NormalizationReport {
    const mappingDetails: Record<string, any> = {};
    const unmappedKeys: any[] = [];
    let mappedCount = 0;
    let unmappedCount = 0;
    let estimatedCount = 0;

    const specs_normalized: SpecsNormalized = {};
    ...
    for (const [rawKey, rawValue] of Object.entries(specs_raw)) {
      ...
      if (normalized.success && normalized.path) {
        ...
        if (!specs_normalized[category as keyof SpecsNormalized]) {
          specs_normalized[category as keyof SpecsNormalized] = {};
        }
        const categoryObj = specs_normalized[category as keyof SpecsNormalized] as any;
  ...
  private static normalizeField(
    ...
    confidence?: number;
    is_estimated?: boolean;
    path?: [string, string]; // [category, fieldKey]
    canonical_key?: string;
    attempted_mapping?: string;
  } {
    // Try semantic mapping first (feature names)
    const semanticResult = SemanticNormalizer.normalize(rawKey);
    if (semanticResult.is_mapped) {
      ...
      return {
        ...
        path: [semanticResult.category as string, semanticResult.canonical_key as string],
   ...
   */
  private static mapRawKeyToNormalized(rawKey: string): {
    category: string;
    key: string;
    type: string;
  } | null {
    // Normalize the key for matching
    const normalized = StringNormalizer.normalizeForLookup(rawKey);
    ...
    const mappings: Record<string, { category: string; key: string; type: string }> = {
      ...
      anti_pinch_windows: { category: 'comfort_convenience', key: 'anti_pinch_windows', type: 'string' },
      'anti-pinch power windows': { category: 'comfort_convenience', key: 'anti_pinch_windows', type: 'string' },
      'anti pinch power windows': { category: 'comfort_convenience', key: 'anti_pinch_windows', type: 'string' },
      'anti pinch windows': { category: 'comfort_convenience', key: 'anti_pinch_windows', type: 'string' },
      ...
      manual_height_adj_seat: { category: 'comfort_convenience', key: 'manual_height_adj_seat', type: 'boolean' },
      'height adj. driver seat': { category: 'comfort_convenience', key: 'manual_height_adj_seat', type: 'boolean' },
      'height adjustable driver seat': { category: 'comfort_convenience', key: 'manual_height_adj_seat', type: 'boolean' },
      'driver seat height adjustment': { category: 'comfort_convenience', key: 'manual_height_adj_seat', type: 'boolean' },
      ...
      upholstery: { category: 'comfort_convenience', key: 'seat_material', type: 'string' },
      'upholstery type': { category: 'comfort_convenience', key: 'seat_material', type: 'string' },
      ...
      turning_radius: { category: 'suspension_steering_brakes', key: 'turning_radius', type: 'string' },
      'turning radius': { category: 'suspension_steering_brakes', key: 'turning_radius', type: 'string' },
      'minimum turning radius': { category: 'suspension_steering_brakes', key: 'turning_radius', type: 'string' },
      ...
      'over speeding alert': { category: 'safety', key: 'speed_alert', type: 'boolean' },
      'overspeeding alert': { category: 'safety', key: 'speed_alert', type: 'boolean' },
      'over speed alert': { category: 'safety', key: 'speed_alert', type: 'boolean' },
      ...
      live_traffic_navigation: { category: 'infotainment_connectivity', key: 'live_traffic_navigation', type: 'boolean' },
      'nav with live traffic': { category: 'infotainment_connectivity', key: 'live_traffic_navigation', type: 'boolean' },
      'live traffic navigation': { category: 'infotainment_connectivity', key: 'live_traffic_navigation', type: 'boolean' },
      'navigation with live traffic': { category: 'infotainment_connectivity', key: 'live_traffic_navigation', type: 'boolean' },
      ...
      'e-call & i-call': { category: 'connected_car', key: 'sos_emergency_assist', type: 'boolean' },
      'e-call': { category: 'connected_car', key: 'sos_emergency_assist', type: 'boolean' },
      'i-call': { category: 'connected_car', key: 'sos_emergency_assist', type: 'boolean' },
      'ecall icall': { category: 'connected_car', key: 'sos_emergency_assist', type: 'boolean' },
      ...
      // New connected car fields
      voice_assistant: { category: 'connected_car', key: 'voice_assistant', type: 'string' },
      'google / alexa connectivity': { category: 'connected_car', key: 'voice_assistant', type: 'string' },
      'google alexa connectivity': { category: 'connected_car', key: 'voice_assistant', type: 'string' },
      'google alexa': { category: 'connected_car', key: 'voice_assistant', type: 'string' },
      valet_mode: { category: 'connected_car', key: 'valet_mode', type: 'boolean' },
      'valet mode': { category: 'connected_car', key: 'valet_mode', type: 'boolean' },
      tow_away_alert: { category: 'connected_car', key: 'tow_away_alert', type: 'boolean' },
      'tow away alert': { category: 'connected_car', key: 'tow_away_alert', type: 'boolean' },
      smartwatch_app: { category: 'connected_car', key: 'smartwatch_app', type: 'boolean' },
      'smartwatch app': { category: 'connected_car', key: 'smartwatch_app', type: 'boolean' },
      live_weather: { category: 'connected_car', key: 'live_weather', type: 'boolean' },
      'live weather': { category: 'connected_car', key: 'live_weather', type: 'boolean' },
      engine_start_alarm: { category: 'connected_car', key: 'engine_start_alarm', type: 'boolean' },
      'engine start alarm': { category: 'connected_car', key: 'engine_start_alarm', type: 'boolean' },
      remote_vehicle_status: { category: 'connected_car', key: 'remote_vehicle_status', type: 'boolean' },
      'remote vehicle status check': { category: 'connected_car', key: 'remote_vehicle_status', type: 'boolean' },
      'remote vehicle status': { category: 'connected_car', key: 'remote_vehicle_status', type: 'boolean' },
      ...
      'halogen / projector type': { category: 'exterior', key: 'headlight_type', type: 'string' },
      'halogen/projector type': { category: 'exterior', key: 'headlight_type', type: 'string' },
      ...
      cornering_fog_lamps: { category: 'exterior', key: 'cornering_fog_lamps', type: 'boolean' },
      'cornering fog lamps': { category: 'exterior', key: 'cornering_fog_lamps', type: 'boolean' },
      'corner fog lamps': { category: 'exterior', key: 'cornering_fog_lamps', type: 'boolean' },
      'cornering fog lights': { category: 'exterior', key: 'cornering_fog_lamps', type: 'boolean' },
      ...
      'gear shift indicator': { category: 'driver_display_controls', key: 'gear_indicator', type: 'boolean' },
  ...
  private static normalizeValueForType(
    value: any,
    type: string
  ): { value: any; confidence: number; is_estimated: boolean } {
    if (type === 'boolean') {
      const result = BooleanNormalizer.normalize(value);
      return {
        value: result.value,
    ...
    if (type === 'number') {
      const numeric = StringNormalizer.extractNumeric(value);
      if (numeric !== null) {
        return {
          value: numeric,
          confidence: 1,
    ...
    if (type === 'string') {
      const result = StringNormalizer.normalize(value);
      return {
