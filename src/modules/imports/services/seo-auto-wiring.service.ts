...
export class SEOAutoWiringService {
   ...
   */
  static async autoWireVariant(
    variant_id: string,
    car_id: string,
    specs_normalized: Record<string, any> | undefined
  ): Promise<void> {
    if (!specs_normalized) {
    ...
    try {
      ...
      if (presets.length > 0) {
        console.log(
          `[SEO Auto-Wiring] Variant ${variant_id} (car: ${car_id}) would wire to: ${presets.join(', ')}`
        );
      }
    } catch (error) {
      // Log but don't fail — SEO auto-wiring is an enhancement
      console.warn(
        `Failed to analyze SEO wiring for variant ${variant_id}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  private static extractEnabledFeatures(specs: Record<string, any>): string[] {
   ...
   */
  static async updateWiringForVariant(
    variant_id: string,
    car_id: string,
    specs_normalized: Record<string, any> | undefined
  ): Promise<void> {
    ...
    try {
      ...
      if (presets.length > 0) {
        console.log(
          `[SEO Auto-Wiring Update] Variant ${variant_id} (car: ${car_id}) updated wiring to: ${presets.join(', ')}`
        );
      }
    } catch (error) {
      console.warn(
        `Failed to update SEO wiring analysis for variant ${variant_id}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
