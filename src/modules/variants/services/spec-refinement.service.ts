...
// A single AI-fillable suggestion for a field flagged as missing by validation.
...
// Metadata for every field that validation can flag as missing. Tells us the
// human label, where the value lives (top-level column vs nested spec path),
// the expected type/unit, and an optional hint to steer the AI.
interface MissingFieldMeta {
  label: string;
  location: 'top' | 'spec';
  path?: string; // dot-path inside specs_normalized (spec location only)
  type: 'number' | 'string' | 'boolean';
  unit?: string;
  hint?: string;
}

const MISSING_FIELD_REGISTRY: Record<string, MissingFieldMeta> = {
...
export class SpecRefinementService {
  private static client: Cerebras | null = null;

  private static getClient(): Cerebras {
    if (!this.client) {
      if (!process.env.CEREBRAS_API_KEY) {
        throw new AppError('CEREBRAS_API_KEY is not set — spec refinement is disabled', 503);
      }
...
Provide 3-5 most impactful suggestions only.`;
  ...
  static async applyRefinementSuggestions(
    ...
    const updatedSpecs = JSON.parse(JSON.stringify((variant as any).specs_normalized || {})) as Record<string, any>;
   ...
   */
  static async suggestMissingFields(variantId: string): Promise<MissingFieldsResult> {
    const variant = await CarVariant.findOne({ variant_id: variantId });
    if (!variant) {
      throw new AppError('Variant not found', 404);
    }
  ...
  private static buildMissingFieldsPrompt(variant: any, car: any, missingKeys: string[]): string {
...
[{"field":"<key>","value":<value>,"reason":"<short reason>","confidence":0.8}]`;
  ...
  private static parseMissingFieldsResponse(
    response: string
  ): Record<string, { value: any; reason: string; confidence: number }> {
    const result: Record<string, { value: any; reason: string; confidence: number }> = {};
   ...
   */
  static async applyMissingFieldValues(
    variantId: string,
    values: Array<{ field: string; value: any }>,
    actor: any = null
  ): Promise<any> {
    const variant = await CarVariant.findOne({ variant_id: variantId });
    if (!variant) {
    ...
    const updatedSpecs = JSON.parse(
      JSON.stringify((variant as any).specs_normalized || {})
    ...
    for (const entry of values) {
      ...
      if (!meta) continue;
      if (entry.value === '' || entry.value === null || entry.value === undefined) continue;

      let coerced: any = entry.value;
      if (meta.type === 'number') {
        coerced = Number(String(entry.value).replace(/[, ]/g, ''));
        if (Number.isNaN(coerced)) continue;
    ...
    if (Object.keys(updatePayload).length === 0 && !specsTouched) {
      throw new AppError('No valid field values to apply', 400);
    }
    ...
    const { CarVariantService } = await import('../../cars/services/car-variant.service');
