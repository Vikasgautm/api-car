import { CarVariant, FieldVisibilityState, SectionVisibility } from '../../../models/car-variant.model';
...
export class VariantLifecycleService {
   ...
   */
  static async markFieldsAsEstimated(variantId: string, fieldKeys: string[]) {
    const variant = await CarVariant.findOne({ variant_id: variantId, is_deleted: false });
    if (!variant) {
      throw new AppError('Variant not found', 404);
    }
   ...
   */
  static async unhideAllSections(variantId: string) {
    const variant = await CarVariant.findOne({ variant_id: variantId, is_deleted: false });
    if (!variant) {
      throw new AppError('Variant not found', 404);
    }
   ...
   */
  static async getUpcomingVariantsNeedingData(carId: string) {
    return await CarVariant.find({
      car_id: carId,
      is_upcoming: true,
      is_deleted: false,
      $or: [
   ...
   */
  static async getEstimationCompleteness(variantId: string) {
    const variant = await CarVariant.findOne({ variant_id: variantId, is_deleted: false });
    if (!variant) {
      throw new AppError('Variant not found', 404);
    }
   ...
   */
  static async createTeaserVariant(variantId: string, teaserData: {
    price?: number;
    launch_date?: Date;
    highlights?: string[];
  }) {
    ...
    variant.section_visibility = sectionKeys.map((key) => ({
      section_key: key,
      visibility: 'teaser_only' as const,
      hidden_fields: [],
    }));
