 ...
 */
// ─── UNIT NORMALIZATION ───────────────────────────────────────────────────────
// Normalises measurement strings from different sources into one canonical form.
// Only fires when the entire value matches a known unit pattern (anchored regex),
// so free-text strings like "Direct Injection DOHC" pass through unchanged.
...
function normalizeUnitString(value: string): string {
...
export function parseSpecValue(value: string, type: SpecMapping['type']): any {
  ...
  switch (type) {
    ...
    case 'boolean': {
      ...
      if (POSITIVE_BOOLEAN_VALUES.has(lower)) return true;
      if (NEGATIVE_BOOLEAN_VALUES.has(lower)) return false;
...
export function guessCategory(label: string, section: string): string {
...
// ============================================================================
// FEATURE INTELLIGENCE — Layer 3
// Derives boolean intelligence flags from normalized + raw specs.
// Output is merged into specs_raw.derived so downstream (SEO tags, filters,
// "cars with X" landing pages) can target a single, consistent shape.
// ============================================================================
...
const stripNoLike = (v: unknown): boolean => {
...
const countTrue = (obj: Record<string, unknown> | undefined): number => {
  if (!obj) return 0;
...
export function deriveFeatureFlags(
  ...
  rootFields?: { fuel_type?: string; transmission_type?: string }
): DerivedFeatureFlags {
  const derived: DerivedFeatureFlags = {};
  ...
  const sunroofPresent =
    stripNoLike(sunroofRaw) || panoramicFlag === true || moonroofFlag === true;
  ...
  if (typeof airbags === 'number' && airbags >= 0) {
    ...
    if (airbags >= 2) derived.has_2_airbags = true;
    if (airbags >= 4) derived.has_4_airbags = true;
    if (airbags >= 6) derived.has_6_airbags = true;
    if (airbags >= 8) derived.has_8_airbags = true;
  }
  ...
  const ncapStars = Math.max(
    typeof safety?.ncap_rating === 'number' ? safety.ncap_rating : 0,
    typeof safety?.bncap_rating === 'number' ? safety.bncap_rating : 0,
    typeof safety?.global_ncap_rating === 'number' ? safety.global_ncap_rating : 0,
    (() => {
      const legacyRaw =
        specs_raw?.bharat_ncap_safety_rating ?? specs_raw?.global_ncap_safety_rating;
      return legacyRaw
  ...
  if (ncapStars > 0) {
    ...
    if (ncapStars >= 5) derived.is_5_star_safety = true;
    else if (ncapStars >= 4) derived.is_4_star_safety = true;
  }

  // ── ADAS ────────────────────────────────────────────────────────────────
  const adas = specs_normalized.adas as Record<string, unknown> | undefined;
  const adasCount = countTrue(adas);
  if (adasCount > 0) {
    derived.has_adas = true;
    derived.adas_count = adasCount;
    if (adasCount >= 5) derived.has_full_adas = true;
  }
  if (adas?.adaptive_cruise_control === true) derived.has_adaptive_cruise = true;

  // ── Lighting ────────────────────────────────────────────────────────────
  const ex = specs_normalized.exterior;
  if (ex?.led_headlights === true) derived.has_led_headlights = true;
  if (ex?.drl === true) derived.has_led_drls = true;
  ...
  if (fuel.includes('hybrid')) derived.is_hybrid = true;
  if (specs_normalized.engine_performance?.turbocharger === true) derived.is_turbo = true;

  // ── Headliner comfort/safety toggles ────────────────────────────────────
  if (
    specs_normalized.infotainment_connectivity?.wireless_charging === true ||
    (specs_normalized.comfort_convenience as any)?.wireless_charger === true
  ) {
  ...
  if (
    specs_normalized.comfort_convenience?.ventilated_seats &&
  ...
  // ── Connected car ───────────────────────────────────────────────────────
  const cc = specs_normalized.connected_car as Record<string, unknown> | undefined;
  const ccCount = countTrue(cc);
  if (ccCount > 0) derived.has_connected_car = true;
  if (cc?.app_connectivity === true) derived.has_app_connectivity = true;
