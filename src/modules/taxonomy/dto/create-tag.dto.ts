...
export class CreateTagDto {
  ...
  static validate(dto: CreateTagDto): { valid: boolean; errors: string[] } {
    const errors: string[] = [];

    const categoryRequired = ValidationUtil.required(dto.tag_category_id, 'tag_category_id');
    if (!categoryRequired.valid) errors.push(...categoryRequired.errors);
