...
export class CarVariantService {
  ...
  static removeHiddenSections(specs_normalized: SpecsNormalized | undefined, hidden_sections: string[] | undefined): SpecsNormalized | undefined {
    ...
    for (const sectionName of sectionsToHide) {
      ...
      if (key && result[key]) {
        ...
        logger.warn(`Unknown section name in hidden_sections: ${sectionName}`);
      }
    }
  ...
  static async getAllVariants(filterDto: any, includeDeleted: boolean = false) {
    try {
      ...
      logger.error('Error in getAllVariants:', error);
      throw new AppError(
  ...
  static async createVariant(variantData: any, actor: AuditActor | null = null) {
    ...
    try {
      ...
      logger.warn(`Failed to auto-wire SEO for variant ${created.variant_id}: ${err instanceof Error ? err.message : String(err)}`);
    }
  ...
  static async cloneVariant(sourceVariantId: string, overrides: { variant_name?: string } = {}, actor: AuditActor | null = null) {
    const source = await CarVariant.findOne({ variant_id: sourceVariantId, is_deleted: { $ne: true } }).lean();
    if (!source) {
      throw new AppError(`Variant not found for clone: ${sourceVariantId}`, 404, {
    ...
    const cloned = await CarVariant.create(cloneData);
    await CarAggregationService.recomputeFullAggregates(cloned.car_id);
  ...
  static async updateVariant(variantId: string, variantData: any, actor: AuditActor | null = null) {
    ...
    if (variantData.specs_normalized !== undefined) {
      ...
      try {
        ...
        logger.warn(`Failed to update SEO wiring for variant ${variant.variant_id}: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  ...
  static async publishVariant(variantId: string, actor: AuditActor | null = null) {
    const variant = await CarVariant.findOneAndUpdate(
      { variant_id: variantId, is_deleted: false },
      { is_published: true, publish_status: 'published' },
      { returnDocument: 'after' }
    );
  ...
  static async unpublishVariant(variantId: string, actor: AuditActor | null = null) {
    const variant = await CarVariant.findOneAndUpdate(
      { variant_id: variantId, is_deleted: false },
      { is_published: false, publish_status: 'draft' },
      { returnDocument: 'after' }
    );
