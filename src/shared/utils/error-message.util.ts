...
export function formatFieldName(field: string): string {
  ...
  const key = String(field).split('.').pop() as string;
 ...
 */
function combineFieldMessage(field: string, message?: string): string {
...
export function normalizeErrors(errors: unknown): string[] {
  ...
  if (Array.isArray(errors)) {
    return errors
      .map((e) => {
        if (e == null) return '';
        if (typeof e === 'string') return cleanValidationMessage(e);
        ...
        if (typeof obj.message === 'string') return cleanValidationMessage(obj.message);
        return '';
      })
      .filter(Boolean);
  }
  ...
  if (anyErr.errors && typeof anyErr.errors === 'object') {
    return Object.values(anyErr.errors)
      .map((e: any) => {
        if (e?.path) return combineFieldMessage(String(e.path), e.message);
