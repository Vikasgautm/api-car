...
export interface CarLaunchStatusPayload {
  ...
  is_latest?: boolean;
  // Support for old typo field
  upcomming?: boolean;
  upcoming?: boolean;
}

export function normalizeCarLaunchStatus(payload: CarLaunchStatusPayload): CarLaunchStatusPayload {
  const normalized: CarLaunchStatusPayload = { ...payload };
...
export function shouldAutoLaunch(car: any): boolean {
...
export function getAutoLaunchUpdateData(): Partial<CarLaunchStatusPayload> {
  const today = new Date();
