...
import { MileageRecomputeService } from './mileage-recompute.service';

// Numeric extraction from messy spec strings.
// "120 bhp @ 6000 rpm" → 120; "250 Nm" → 250; "200 mm" → 200; "8.5 sec" → 8.5.
// Returns null if no number found.
const extractNumber = (raw: unknown): number | null => {
...
const normalizeDriveLabel = (raw: string | null | undefined): string | null => {
  if (!raw) return null;
...
export interface CarAggregates {
  ...
  // AI intelligence (deterministic rules; hybrid LLM refinement in CarIntelligenceLLMService)
  ...
  // Audit subdoc: per-flag confidence (0..1) and rationale, plus refined_by_llm trail.
  // Confidence is |signals_fired/total_signals - 0.5| * 2 → 1.0 means clear yes/no,
  // 0.0 means the rule was ambivalent.
  ai_intelligence_meta: {
    confidence_scores: Record<AiFlagKey, number>;
    flag_rationale: Record<AiFlagKey, string>;
    // refined_by_llm is preserved across rule recomputes via applyAggregates merge.
...
export const AI_FLAG_KEYS: AiFlagKey[] = [
  'family_friendly',
  'city_friendly',
  'highway_friendly',
  'offroad_ready',
...
export const AMBIGUOUS_CONFIDENCE_THRESHOLD = 0.5;
...
function scoreSignals(
  ...
  total: number,
  threshold: number,
): { verdict: boolean; confidence: number } {
  if (total <= 0) return { verdict: false, confidence: 0 };
...
export class CarAggregationService {
  /**
   ...
   */
  static async computeAggregates(carId: string): Promise<CarAggregates | null> {
  ...
  static computeAggregatesFromVariants(variants: Array<Partial<ICarVariant>>): CarAggregates {
    const agg: CarAggregates = {
      ...
      ai_intelligence_meta: {
        confidence_scores: {} as Record<AiFlagKey, number>,
        flag_rationale: {} as Record<AiFlagKey, string>,
        refined_by_llm: [],
    ...
    const batterySet = new Set<number>();
    ...
    for (const v of variants) {
      ...
      if (v.fuel_type_id) fuelIdSet.add(v.fuel_type_id);
      ...
      if (engine.displacement || engine.max_power) {
        ...
        if (parts.length > 0) engineSet.add(parts.join(' ').trim());
      }

      // Battery capacities
      const batteryKwh =
        typeof battery.battery_capacity_kwh === 'number'
          ? battery.battery_capacity_kwh
          : extractNumber(battery.battery_capacity);
      if (batteryKwh != null && batteryKwh > 0) batterySet.add(batteryKwh);
      ...
      const mileageValue =
        typeof v.mileage_class_value === 'number'
      ...
      // Range (EVs)
      const rangeValue =
        typeof v.range_class_value === 'number'
          ? v.range_class_value
      ...
      if (gc != null) agg.ground_clearance_mm = maxNonNull(agg.ground_clearance_mm, gc);
      const bs = extractNumber(dims.boot_space);
      if (bs != null) agg.boot_space_l = maxNonNull(agg.boot_space_l, bs);
      const wb = extractNumber(dims.wheelbase);
      if (wb != null) agg.wheelbase_mm = maxNonNull(agg.wheelbase_mm, wb);

      const seating =
        typeof dims.seating_capacity === 'number'
      ...
      if (interior.panoramic_sunroof) agg.panoramic_sunroof_available = true;
      ...
      if (adasFlags > 0) agg.adas_available = true;
      totalAdasCount = Math.max(totalAdasCount, adasFlags);

      if (comfort.ventilated_seats) agg.ventilated_seats_available = true;
      if (safety.camera_360) agg.camera_360_available = true;
      if (
      ...
      if (infotainment.wireless_charging) agg.wireless_charger_available = true;
      if (comfort.air_quality_control) agg.air_purifier_available = true;
      ...
      if (bodyType.includes('suv') || bodyType.includes('crossover')) hasSuv = true;
    }
    ...
    const confidence_scores = {} as Record<AiFlagKey, number>;
    const flag_rationale = {} as Record<AiFlagKey, string>;
    ...
    const familySignals = [
      ...
      {
        hit:
          variants.some(v => v.specs_normalized?.comfort_convenience?.rear_ac_vents) &&
    ...
    const hasCompactBody = Array.from(allBodyTypes).some(
      bt => bt.includes('hatch') || bt.includes('sedan'),
    );
    const lengthMax = variants.reduce<number | null>((acc, v) => {
    ...
    const fastest = variants.reduce<number | null>((acc, v) => {
    ...
    (agg as any).__fuelIds = Array.from(fuelIdSet);
   ...
   */
  static async applyAggregates(carId: string, agg: CarAggregates): Promise<void> {
    const fuelIds: string[] = (agg as any).__fuelIds ?? [];
    let aggregated_fuel_types: string[] = [];
    if (fuelIds.length > 0) {
      const fuelDocs = await FuelType.find({ fuel_type_id: { $in: fuelIds }, is_deleted: false })
    ...
    const prior = await Car.findOne({ car_id: carId })
    ...
    if (priorMeta?.refined_by_llm && priorMeta.refined_by_llm.length > 0) {
      const stillAmbiguous: AiFlagKey[] = [];
      for (const flag of priorMeta.refined_by_llm as AiFlagKey[]) {
        const ruleConfidence = agg.ai_intelligence_meta.confidence_scores[flag] ?? 0;
        if (ruleConfidence < AMBIGUOUS_CONFIDENCE_THRESHOLD) {
          // LLM verdict still load-bearing — restore it.
          const llmVerdict = (priorMeta as any)[flag] as boolean | undefined;
          if (typeof llmVerdict === 'boolean') {
            (agg as any)[flag] = llmVerdict;
          }
    ...
    const writable: Record<string, unknown> = { ...(agg as unknown as Record<string, unknown>) };
    delete writable.__fuelIds;

    await Car.updateOne(
      { car_id: carId },
      { $set: { ...writable, aggregated_fuel_types } }
    );
  }
   ...
   */
  static async recomputeFullAggregates(carId: string): Promise<CarAggregates | null> {
    ...
    await MileageRecomputeService.recomputeCarAggregatesOnly(carId);
    return aggregates;
  }
}
