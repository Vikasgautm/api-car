import { SUBCATEGORY_DISPLAY_NAMES } from './media-constants';

export class MediaSeoService {
  /**
  ...
  static generateTitle(carName: string, subCategory: string): string {
    const subcatLabel = SUBCATEGORY_DISPLAY_NAMES[subCategory] || subCategory;
   ...
   */
  static buildSeoFields(
    carName: string,
    subCategory: string,
    existingAlt?: string,
    existingTitle?: string,
  ): { alt_text: string; image_title: string } {
    return {
