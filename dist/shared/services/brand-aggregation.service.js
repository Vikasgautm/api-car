...
exports.BrandAggregationService = void 0;
...
function fuelSlug(fuelName) {
    const lower = fuelName.toLowerCase().trim();
    return FUEL_SLUG_MAP[lower] || toSlug(fuelName);
}
class BrandAggregationService {
    static async computeAndCache(brandId) {
        const brand = await brand_model_1.Brand.findOne({ brand_id: brandId, is_deleted: false }).lean();
        if (!brand)
            return null;
        const cars = await car_model_1.Car.find({ brand_id: brandId, is_deleted: false })
            .select('car_id name body_type_id body_type_name aggregated_fuel_types ' +
        ...
        if (carIds.length === 0) {
            ...
            await brand_model_1.Brand.updateOne({ brand_id: brandId }, { $set: { aggregates_cache: empty } });
            return empty;
        }
        const totalVariants = await car_variant_model_1.CarVariant.countDocuments({
            car_id: { $in: carIds },
            is_deleted: false,
        });
        ...
        for (const car of cars) {
            const typeId = car.body_type_id || 'unknown';
            const typeName = car.body_type_name || typeId;
            ...
            entry.count++;
            if (entry.cars.length < 5)
                entry.cars.push(car.name);
        }
        ...
        for (const car of cars) {
            const fuels = Array.isArray(car.aggregated_fuel_types) ? car.aggregated_fuel_types : [];
        ...
        const price_range = minPrice !== Infinity
        ...
        let sunroofCount = 0;
        for (const car of cars) {
            if (car.adas_available)
                adasCount++;
            if (car.sunroof_available)
                sunroofCount++;
        }
        ...
        await brand_model_1.Brand.updateOne({ brand_id: brandId }, { $set: { aggregates_cache: result } });
        return result;
    }
    static async refreshAll() {
        const brands = await brand_model_1.Brand.find({ is_deleted: false }).select('brand_id').lean();
        let processed = 0;
        let errors = 0;
