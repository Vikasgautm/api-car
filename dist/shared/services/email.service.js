...
class EmailService {
    ...
    static isConfigured() {
        return Boolean(config_1.config.email.smtp_host && config_1.config.email.smtp_user && config_1.config.email.smtp_pass);
    }
    static getTransporter() {
        if (this.transporter)
            return this.transporter;
        this.transporter = nodemailer_1.default.createTransport({
            host: config_1.config.email.smtp_host,
            port: config_1.config.email.smtp_port,
            secure: config_1.config.email.smtp_secure,
            auth: { user: config_1.config.email.smtp_user, pass: config_1.config.email.smtp_pass },
        });
        return this.transporter;
    }
    /**
    ...
    static async sendOtp(recipient, otp, ctx) {
        ...
        const ttlMinutes = Math.round(config_1.config.deletion_workflow.otp_ttl_seconds / 60);
        const lines = [
    ...
    static async sendLifecycleOtp(recipient, otp, ctx) {
        ...
        const ttlMinutes = Math.round(config_1.config.lifecycle_governance.otp_ttl_seconds / 60);
