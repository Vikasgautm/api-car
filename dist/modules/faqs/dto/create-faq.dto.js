...
class CreateFaqDto {
    ...
    static validate(dto) {
        const errors = [];
        const qResult = validation_util_1.ValidationUtil.required(dto.question, 'question');
        if (!qResult.valid)
            errors.push(...qResult.errors);
        if (dto.question && dto.question.length < 10)
            errors.push('question must be at least 10 characters long');
        const aResult = validation_util_1.ValidationUtil.required(dto.answer, 'answer');
        if (!aResult.valid)
            errors.push(...aResult.errors);
        if (dto.answer && dto.answer.length < 20)
            errors.push('answer must be at least 20 characters long');
        const cResult = validation_util_1.ValidationUtil.required(dto.category, 'category');
        if (!cResult.valid)
