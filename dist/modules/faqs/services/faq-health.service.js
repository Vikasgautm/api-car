...
exports.FAQHealthService = void 0;
...
async function entityExists(entityType, entityId) {
    ...
    try {
        switch (entityType) {
            case 'car':
                return !!(await car_model_1.Car.exists({ car_id: entityId, is_deleted: false }));
            case 'brand':
                return !!(await brand_model_1.Brand.exists({ brand_id: entityId, is_deleted: false }));
            case 'variant':
                return !!(await car_variant_model_1.CarVariant.exists({ variant_id: entityId, is_deleted: false }));
            case 'body_type':
                return !!(await body_type_model_1.BodyType.exists({ body_type_id: entityId, is_deleted: false }));
            case 'fuel_type':
                return !!(await fuel_type_model_1.FuelType.exists({ fuel_type_id: entityId, is_deleted: false }));
...
class FAQHealthService {
     ...
     */
    static async checkFaq(faqId) {
        const faq = await faq_model_1.FAQ.findOne({ faq_id: faqId, is_deleted: false }).lean();
        if (!faq)
        ...
        const needs_refresh = faq_health_score < 60 || freshness_score < 50 || danglingCount > 0 || plainAnswer.length < THIN_ANSWER_THRESHOLD;
        await faq_model_1.FAQ.updateOne({ faq_id: faqId }, { $set: { faq_health_score, freshness_score, needs_refresh } });
     ...
     */
    static async runBulkHealthCheck(limit = 500) {
        const faqs = await faq_model_1.FAQ.find({ is_deleted: false }).select('faq_id').limit(limit).lean();
        const results = await Promise.allSettled(faqs.map((f) => FAQHealthService.checkFaq(f.faq_id)));
