...
exports.SeoFilterGeneratorService = void 0;
const car_variant_model_1 = require("../../../models/car-variant.model");
const seo_preset_model_1 = require("../../../models/seo-preset.model");
const seo_preset_service_1 = require("./seo-preset.service");
class SeoFilterGeneratorService {
    /**
     ...
     */
    static async generateFeatureAvailability() {
        ...
        for (const variant of variants) {
            for (const field of featureFields) {
                ...
                if (value === true || value === 'yes' || (value && typeof value === 'string' && value.toLowerCase() !== 'no')) {
                    ...
                    if (variant.car_id) {
                        const carId = variant.car_id._id?.toString() || variant.car_id.toString();
                        stat.cars.add(carId);
                        stat.brands.add(variant.car_id.brand_id?.toString() || '');
                    }
                }
            }
        }
        // Convert to output format, sorted by variant count
        const output = Array.from(results.entries())
     ...
     */
    static async autoGeneratePresets(minVariantCount = 5) {
        const features = await this.generateFeatureAvailability();
        const created = [];
        // Batch fetch all existing presets to avoid N findOne queries in loop
        const existingPresets = await seo_preset_model_1.SeoPreset.find({ is_deleted: false }).select('slug').lean();
        const existingSlugs = new Set(existingPresets.map((p) => p.slug));
        for (const feature of features) {
            if (feature.variant_count < minVariantCount)
                continue;
            ...
            try {
                const preset = await seo_preset_service_1.SeoPresetService.create({
                    slug: feature.slug,
