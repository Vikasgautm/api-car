...
class CityService {
    ...
    static async updateCity(cityId, cityData) {
        ...
        if (cityData.pincode !== undefined)
            updateData.pincode = cityData.pincode != null ? String(cityData.pincode) : undefined;
    ...
    static async bulkSeedCities() {
        let inserted = 0;
        let skipped = 0;
        for (const c of ALL_INDIA_CITIES) {
            const slug = slug_util_1.SlugUtil.generate(`${c.name}-${c.state}`);
            const exists = await city_model_1.City.findOne({ slug });
            if (exists) {
                skipped++;
                continue;
            }
            await city_model_1.City.create({
                city_id: (0, uuid_1.v4)(),
                name: c.name,
                slug,
                state: c.state,
