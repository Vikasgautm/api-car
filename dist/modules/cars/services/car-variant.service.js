...
class CarVariantService {
    static async getAllVariants(query, fetchAsAdmin = false) {
        ...
        return { variants, total, page: Number(page) || 1, limit: Number(limit) || 10 };
    }
    ...
    static async updateVariant(id, variantData) {
        ...
        return await car_variant_model_1.CarVariant.findByIdAndUpdate(id, variantData, { new: true });
    }
    static async deleteVariant(id) {
        return await car_variant_model_1.CarVariant.findByIdAndUpdate(id, { is_deleted: true }, { new: true });
    }
    static async restoreVariant(id) {
        return await car_variant_model_1.CarVariant.findByIdAndUpdate(id, { is_deleted: false }, { new: true });
    }
}
