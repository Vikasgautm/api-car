...
class CarVariantService {
    ...
    static async getAllVariants(filterDto, includeDeleted = false) {
        try {
            ...
            if (q) {
                ...
                const escaped = filter_util_1.FilterUtil.escapeRegExp(qStr);
                const flexiblePattern = escaped.replace(/\s+/g, '[\\s-_]+');
                const searchRegex = new RegExp(flexiblePattern, 'i');
                const tokens = qStr.split(/\s+/).filter(t => t.length > 1).map(t => filter_util_1.FilterUtil.escapeRegExp(t));
                ...
                const brandIds = brandMatches.map((b) => b.brand_id);
                const carMatches = await car_model_1.Car.find({
                    $or: [
                        { name: searchRegex },
                        { slug: searchRegex },
                ...
                const carIds = carMatches.map((c) => c.car_id);
                const orClauses = [
                ...
                filter_util_1.FilterUtil.mergeFilterWithOr(filter, { $or: orClauses });
            }
    ...
    static async getGroupedVariants(filterDto) {
        ...
        if (q) {
            ...
            const carIdsByName = carMatches.map((c) => c.car_id);
            const orTerms = [
                { variant_name: searchRegex },
                { slug: searchRegex },
                { trim_name: searchRegex },
        ...
        if (hasVariantLevelFilters) {
            ...
            const brandIds = brandMatches.map((b) => b.brand_id);
            const carNameMatches = await car_model_1.Car.find({
                $or: [
                    { name: searchRegex },
                    { slug: searchRegex },
                    { model_family: searchRegex },
        ...
        const transformedVariants = await Promise.all(allVariants.map(async (v) => {
            try {
                return await variant_response_transformer_1.VariantResponseTransformer.transform(v);
            }
            catch (err) {
                logger_1.logger.warn(`Failed to transform variant ${v.variant_id} — returning degraded object`, { err });
    ...
    static async buildUniqueVariantSlug(carSlug, variantName, excludeVariantId) {
        const namePart = slug_util_1.SlugUtil.generate(variantName);
        const carPart = carSlug ? slug_util_1.SlugUtil.generate(carSlug) : '';
        const baseSlug = carPart ? `${carPart}-${namePart}` : namePart;
        const pattern = new RegExp(`^${baseSlug}(-\\d+)?$`);
        const query = { slug: pattern, is_deleted: false };
        if (excludeVariantId) {
            query.variant_id = { $ne: excludeVariantId };
        }
        const existingSlugs = (await car_variant_model_1.CarVariant.find(query).select('slug').lean()).map((v) => v.slug);
        return existingSlugs.includes(baseSlug)
            ? slug_util_1.SlugUtil.generateUnique(baseSlug, existingSlugs)
            : baseSlug;
    }
    static async createVariant(variantData, actor = null) {
        ...
        variantData.slug = await this.buildUniqueVariantSlug(car.slug, variantData.variant_name);
        const variant = {
            variant_id,
    ...
    static async cloneVariant(sourceVariantId, overrides = {}, actor = null) {
        ...
        const car = await car_model_1.Car.findOne({ car_id: source.car_id, is_deleted: false }).select('slug').lean();
        const uniqueSlug = await this.buildUniqueVariantSlug(car?.slug, baseName);
        const cloneData = {
            ...source,
    ...
    static async updateVariant(variantId, variantData, actor = null) {
        ...
        if (variantData.variant_name !== undefined || variantData.car_id !== undefined) {
            const effectiveCarId = updateData.car_id ?? before?.car_id;
            const effectiveName = updateData.variant_name ?? before?.variant_name;
            if (effectiveCarId && effectiveName) {
                const car = await car_model_1.Car.findOne({ car_id: effectiveCarId, is_deleted: false }).select('slug').lean();
                updateData.slug = await this.buildUniqueVariantSlug(car?.slug, effectiveName, variantId);
            }
        }
