...
exports.SpecNormalizationService = void 0;
class SpecNormalizationService {
    static normalizeMileage(raw) {
    ...
    static normalizeDisplacement(raw) {
        if (!raw)
            return undefined;
        const s = raw.toLowerCase().trim();
        ...
        if (ccMatch)
            return `${parseFloat(ccMatch[1])} cc`;
        if (litreMatch) {
    ...
    static normalizeTransmission(raw) {
        if (!raw)
            return undefined;
        const s = raw.toLowerCase().trim();
        if (s.includes('manual') && !s.includes('auto'))
            return 'Manual';
        if (s.includes('automatic') || s.includes('auto'))
            return 'Automatic';
        if (s.includes('amt'))
            return 'AMT';
        if (s.includes('cvt'))
    ...
    static normalizeSpecs(rawSpecs) {
        const result = {};
