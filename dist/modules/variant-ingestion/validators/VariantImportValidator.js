...
exports.VariantImportValidator = void 0;
...
class VariantImportValidator {
    ...
    static validateSpecs(specs, issues) {
        // Mileage checks
        ...
        if (mileage !== undefined) {
            ...
            if (!isNaN(val)) {
                if (val < 0)
                    issues.push({ field: 'mileage', message: 'Mileage cannot be negative', severity: 'error' });
                else if (val > 100)
                    issues.push({ field: 'mileage', message: `Mileage of ${val} kmpl seems unrealistic`, severity: 'warning' });
            }
        }
        ...
        if (power !== undefined) {
            ...
            if (!isNaN(val)) {
                if (val <= 0)
                    issues.push({ field: 'max_power', message: 'Power must be positive', severity: 'error' });
                else if (val > 2000)
                    issues.push({ field: 'max_power', message: `Power value ${val} bhp seems unrealistic`, severity: 'warning' });
            }
        }
